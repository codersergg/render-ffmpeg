...
object AssBuilder {
    ...
    fun buildAssFile(
        ...
        /** СТИЛЬ С ФОНОМ ПОД СТРОКОЙ (BorderStyle=3, BackColour из OverlayStyle.box*) */
        fun styleLine(name: String, ls: OverlayLineStyle, forceBold: Boolean, minOpacity: Double) = buildString {
            ...
            val back = rgbaToAss(style.boxOpacity, style.boxColor.ifBlank { "#000000" })
            append("Style: $name,${style.fontFamily},${style.fontSizePx},")
            append("$primary,&H000000FF,&H00000000,$back,")
            append("${if (forceBold) -1 else 0},0,0,0,100,100,0,0,")
            append("3,$outline,$shadow,")
